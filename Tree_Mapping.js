...
Tree.prototype.map = function (callback) {
  ...
  //when each node is passed in, create a new tree
  //pass the cb on the new tree's value
  function recurse(node){
    var mappedTree = new Tree(callback(node.value));
    var kids = node.children;
    for(var i = 0; i < kids.length; i++){
      mappedTree.children.push(recurse(kids[i]));
    }
    return mappedTree;
  }
   return recurse(this);
